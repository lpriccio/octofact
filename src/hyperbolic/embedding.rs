...
/// `bowl_height` controls the asymptotic maximum Y (default 0.4).
/// Matches the shader's disk_to_bowl_h function.
///
/// Returns [X, Y, Z] where Y is "up".
pub fn disk_to_bowl(z: Complex, bowl_height: f64) -> [f32; 3] {
    let r2 = z.norm_sq();
    let y = bowl_height * r2 / (1.0 + r2);
...
mod tests {
    ...
    fn test_origin_maps_to_center() {
        let [x, y, z] = disk_to_bowl(Complex::ZERO, 0.4);
    ...
    fn test_bowl_bounded() {
        ...
        for p in &test_points {
            let [_x, y, _z] = disk_to_bowl(*p, 0.4);
            assert!(
                y < 0.5,
    ...
    fn test_y_increases_with_distance() {
        let near = disk_to_bowl(Complex::new(0.1, 0.0), 0.4);
        let far = disk_to_bowl(Complex::new(0.5, 0.0), 0.4);
    ...
    fn test_xz_match_disk_coords() {
        ...
        let [x, _y, zz] = disk_to_bowl(z, 0.4);
